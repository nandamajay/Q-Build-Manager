...
<head>
    <meta charset="UTF-8">
    <title>Q-Build Manager V6</title>
    ...
    <script>hljs.highlightAll(); if (Notification.permission !== "granted") Notification.requestPermission();</script>
</head>
<body class="bg-gray-900 text-gray-100 font-sans min-h-screen flex flex-col">
    <nav class="bg-gray-800 p-4 border-b border-gray-700">
        <div class="container mx-auto flex justify-between items-center">
            <a href="/" class="text-2xl font-bold text-blue-400"><i class="fas fa-microchip mr-2"></i>Q-Build <span class="text-xs text-green-400">STABLE</span></a>
...
<div class="flex h-[80vh] bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700">
    ...
    <div class="w-3/4 flex flex-col bg-[#282c34]">
        ...
        <div class="flex-grow overflow-auto p-4">{% if is_file %}<pre><code class="language-{{ ext }}">{{ content }}</code></pre>{% else %}<div class="flex items-center justify-center h-full text-gray-500"><div class="text-center"><i class="fas fa-code text-6xl mb-4 opacity-20"></i><p>Select a file to view content</p></div></div>{% endif %}</div>
    </div>
</div>
"""

# Templates for Wizard (Step 1 & 2) and Build Console
...
CREATE_STEP2_HTML = """<div class="max-w-2xl mx-auto bg-gray-800 p-8 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-6">Step 2: Configuration</h2><form action="/finish_create" method="POST" class="space-y-6"><input type="hidden" name="name" value="{{ project }}"><div><label class="block text-sm text-gray-400 mb-1">Target Board</label><select name="board" class="w-full bg-gray-900 border border-gray-600 rounded p-3 text-white">{% for b in boards %}<option value="{{ b }}">{{ b }}</option>{% endfor %}</select></div><div><label class="block text-sm text-gray-400 mb-1">Topology</label><div class="flex space-x-4"><label class="flex items-center space-x-2 bg-gray-900 p-3 rounded border border-gray-600 flex-1"><input type="radio" name="topology" value="ASOC" checked> <span>ASOC (qcom-distro.yml)</span></label><label class="flex items-center space-x-2 bg-gray-900 p-3 rounded border border-gray-600 flex-1"><input type="radio" name="topology" value="AudioReach"> <span>AudioReach (prop-image.yml)</span></label></div></div><button type="submit" class="w-full bg-green-600 hover:bg-green-500 py-3 rounded font-bold mt-4">Create Project</button></form></div>"""
BUILD_CONSOLE_HTML = """<div class="flex flex-col h-full space-y-4"><div class="bg-gray-800 p-4 rounded-lg shadow flex justify-between items-center"><div><h2 class="text-2xl font-bold">{{ project }}</h2><div class="text-sm text-gray-400 mt-1">Status: <span id="statusBadge" class="font-bold">UNKNOWN</span></div><div class="text-xs text-gray-500 font-mono" id="cmdInfo"></div></div><div class="flex space-x-3 items-center"><button onclick="stopBuild()" id="stopBtn" class="hidden bg-red-600 text-white px-6 py-2 rounded">STOP</button><button onclick="startBuild()" id="buildBtn" class="bg-green-600 text-white px-6 py-2 rounded">BUILD</button><a href="/" class="bg-gray-700 px-4 py-2 rounded text-white">Back</a></div></div><div id="terminal" class="flex-grow bg-black rounded h-[600px]"></div></div><script>var socket = io(); var project = '{{ project }}'; var term = new Terminal({theme:{background:'#000',foreground:'#e5e5e5'}}); var fitAddon = new FitAddon.FitAddon(); term.loadAddon(fitAddon); term.open(document.getElementById('terminal')); fitAddon.fit(); socket.emit('join_project', {project:project}); socket.on('log_chunk', function(msg){ term.write(msg.data); }); socket.on('build_status', function(msg){ updateUI(msg.status); }); socket.on('cmd_info', function(msg){ document.getElementById('cmdInfo').innerText = "Command: " + msg.cmd; }); function updateUI(status){ var b=document.getElementById('buildBtn'); var s=document.getElementById('stopBtn'); document.getElementById('statusBadge').innerText=status.toUpperCase(); if(status=='running'){b.classList.add('hidden'); s.classList.remove('hidden');}else{b.classList.remove('hidden'); s.classList.add('hidden');}} function startBuild(){term.clear(); socket.emit('start_build',{project:project});} function stopBuild(){socket.emit('stop_build',{project:project});}</script>"""

# --- ROUTES (Identical to V5, mostly) ---
@app.route('/')
def index():
...
def finish_create():
    ...
    # Storing just the board file; we calculate distro file dynamically during build
...
def delete(name):
    ...
    if name in reg:
        ...
        del reg[name]; save_registry(reg)
    return redirect('/')
...
def handle_build(data):
    ...
    # --- LOGIC FIX HERE ---
    board_yml = cfg.get('kas_files')
    topology = cfg.get('topology', 'ASOC')
    
    # Select Distro Config based on Topology
    if topology == 'AudioReach':
        distro_yml = 'meta-qcom/ci/qcom-distro-prop-image.yml'
    else:
        distro_yml = 'meta-qcom/ci/qcom-distro.yml'
    
    # Construct KAS args: file1:file2
    kas_args = f"{board_yml}:{distro_yml}"
    ...
    emit('cmd_info', {'cmd': cmd}, to=name) # Show user the exact command being run
